...
def test_html_section_chunking_does_not_leak_the_explanatory_comment():
    chunks = [c for c in build_all_chunks() if c[1]["doc_type"] == "cybertronian_protocols"]
    assert len(chunks) == 3
    titles = {meta["title"] for _text, meta in chunks}
    assert titles == {"Protocol: Autobot Mobilization", "Protocol: Teletraan Purge", "Protocol: Omega Supreme Deployment"}
...
def test_markdown_header_chunking_keeps_heading_with_its_body():
    chunks = [c for c in build_all_chunks() if c[1]["doc_type"] == "campaign_records"]
    assert len(chunks) == 4
    for text, meta in chunks:
...
def test_csv_row_chunking_binds_values_to_column_headers():
    chunks = [c for c in build_all_chunks() if c[1]["doc_type"] == "repair_bay_logs"]
    assert len(chunks) == 5
    first_text, _meta = chunks[0]
    assert "Scout Chassis" in first_text
    assert "Ratchet" in first_text


def test_json_record_chunking_produces_one_chunk_per_array_element():
    chunks = [c for c in build_all_chunks() if c[1]["doc_type"] == "autobot_command_roster"]
    assert len(chunks) == 4
    names = {meta["name"] for _text, meta in chunks}
    assert "Elita One" in names
