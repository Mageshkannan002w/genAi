...
structured DB. No API keys, no cloud services, no cost.
...
2. [Architecture](#2-architecture)
   ...
   - 2.2 [Structured fleet-ops DB & the NL2SQL pipeline](#22-structured-fleet-ops-db--the-nl2sql-pipeline)
...
9. [Make It Your Own (the intern assignment)](#9-make-it-your-own-the-intern-assignment)
...
- **Agentic mode** — OPTIMUS gets a `TOOL_REGISTRY` (vector search, SQL fleet-database
  queries, alerts, reminders, memory) and a ReAct planning loop. The UI shows his full
  Thought → Action → Observation trace, including any SQL he generated, for every reply.
...
guard.validate_select_only()   <- rejects non-SELECT / multiple statements / banned
    ...
    ▼
readonly_engine.execute()   <- runs as the `optimus_readonly` Postgres role, which has
...
│   ├── setup_db.py               # create fleet tables + the optimus_readonly role
...
python -m scripts.setup_db     # creates the 4 tables + the read-only optimus_readonly role
...
- **`role "optimus" does not exist` / setup_db connects but to the wrong database** — you
...
1. Edit `OPTIMUS_SYSTEM_PROMPT` in `config/settings.py` — change the voice, the name, the
