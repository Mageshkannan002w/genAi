"""
src/core/chunking.py - different chunking strategies for different OPTIMUS knowledge types.
...
def chunk_by_csv_row(filename, doc_type):
    ...
    with open(path, "r", encoding="utf-8", newline="") as f:
        for row in csv.DictReader(f):
            flat = (
                f"On {row['Date']}, {row['Chassis']}'s {row['Component']} had an issue: "
...
INGESTION_PLAN = [
    (chunk_by_sentence, "leadership_quips.txt", "leadership_quips"),
    (chunk_by_paragraph, "prime_directives.txt", "prime_directives"),
    (chunk_by_paragraph, "base_operations.txt", "base_operations"),
    (chunk_by_record, "chassis_telemetry.txt", "chassis_telemetry"),
    (chunk_by_procedure, "engagement_tactics.txt", "engagement_tactics"),
    (chunk_by_markdown_header, "campaign_records.md", "campaign_records"),
    (chunk_by_json_record, "autobot_command_roster.json", "autobot_command_roster"),
    (chunk_by_csv_row, "repair_bay_logs.csv", "repair_bay_logs"),
    (chunk_by_html_section, "cybertronian_protocols.html", "cybertronian_protocols"),
]
